import {useStyles} from "./productColorIntefaceCss"
...
export default function ColourInterface() {
    ...
    const handleClick = async () => {
        ...
        if (productColorName.length == 0) {
            err = true
            handleErrorMessage('productColorName', 'Please Input Colour...')
        }
        if (err==false) {
            ...
            var result =await postData('productColors/insert_productColors', formData)
            if (result.status) {
                Swal.fire({
                    icon: "success",
            ...
            <div className={classes.box}>
                ...
                <div style={{ margin: 8 }}>
                    <Grid2 container spacing={2}>
                        ...
                        <Grid2 size={6} >
                            <TextField value={productColor} error={error.productColor} helperText={error.productColor} onFocus={() => handleErrorMessage('productColor', null)} label="Colour Code" onChange={(e) => setProductColor(e.target.value)} fullWidth />
                        </Grid2>
                        <Grid2 size={6} >
                            <TextField value={productColorName} error={error.productColorName} helperText={error.productColorName} onFocus={() => handleErrorMessage('productColorName', null)} label="Colour" onChange={(e) => setProductColorName(e.target.value)} fullWidth />
                        </Grid2>
